...
async function getGlobalIntegrationsCollection() {
	...
	const collection = await service
		.getCollection('Integrations', 'Integration')
		.refresh();
...
export default async function getIntegrationsCollection(context) {
	return context && context.hasLink('Integrations')
		? getContextIntegrationsCollection(context)
		: getGlobalIntegrationsCollection();
}
